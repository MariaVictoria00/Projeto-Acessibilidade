<template>
   <table  class="table table-borderless table-responsive" style="width:100%">
  <thead class="responsive">
    <tr>
      <th tabindex="1" scope="col">Nome</th>
      <th tabindex="2" scope="col">CPF</th>
      <th tabindex="3" scope="col">Cidade</th>
      <th tabindex="0" scope="col">UF</th>
      <th tabindex="0" scope="col">Bairro</th>
      <th tabindex="0" scope="col">Endereço</th>
      <th tabindex="0" scope="col">Telefone</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">Andressa dos santos</th>
      <td>147852369</td>
      <td>Corumbá</td>
      <td>MS</td>
      <td>Universitario</td>
      <td>Dom aquino</td>
      <td>3335896</td>
    </tr>
    <tr>
      <th scope="row">Joao da Silva</th>
      <td>258963</td>
      <td>Ladário</td>
      <td>MS</td>
      <td>Centro</td>
      <td>14 de marco</td>
      <td>32310412</td>
    </tr>
    <tr>
    <th scope="row">Maria</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Michael</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Bruno</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Lucas</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Tatiana</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">MJessica</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Ronaldo</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Ana Calorina</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Joana Pereira</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Marciel</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
       <tr>
    <th scope="row">Mariana</th>
        <td>258963</td>
        <td>Ladário</td>
        <td>MS</td>
        <td>Centro</td>
        <td>14 de marco</td>
        <td>32310412</td>
      </tr>
  </tbody>
</table>




</template>

  
